...
def metric_cal(scores, gt_list, gt_mask_list, cal_pro=False):
    ...
    thresholds = np.linspace(min(img_scores), max(img_scores), num=100)
    max_accuracy = 0
    optimal_threshold = 0
    for threshold in thresholds:
        # 将分数转换为0或1的预测，根据是否超过阈值
        predictions = [1 if score >= threshold else 0 for score in img_scores]
        accuracy = accuracy_score(gt_list, predictions)
        if accuracy > max_accuracy:
            max_accuracy = accuracy
            optimal_threshold = threshold
    # 使用最佳阈值计算 F1 分数、精确率和召回率
    optimal_predictions = [1 if score >= optimal_threshold else 0 for score in img_scores]
    f1 = f1_score(gt_list, optimal_predictions)
    precision = precision_score(gt_list, optimal_predictions)
    recall = recall_score(gt_list, optimal_predictions)
