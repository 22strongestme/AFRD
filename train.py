...
def train_epoch(model: CoverModel, dataloader: DataLoader, optimizer: torch.optim.Optimizer, device: str, **kwargs):
    ...
    for batch in dataloader:
        # batch[0] is a tensor of shape [batch_size, 6, channels, height, width]
        stacked_images, labels, masks = batch
        stacked_images = stacked_images.to(device)
        outputs = model(stacked_images)  # If the model expects six images, it's ready to be used here.
        loss = model.cal_loss(**outputs)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def test_epoch(model:CoverModel, dataloader:DataLoader, device:str, is_vis, img_dir, class_name, cal_pro, **kwargs):
    ...
    for batch in dataloader:
        ...
        # Process each set of stacked images through the model
        outputs = model(stacked_images)
        score = model.cal_am(**outputs)
        scores.extend(score)

        # Store ground truth labels and masks
        gt_list.extend(labels.cpu().numpy())
        for mask_tensor in masks:
            gt_mask_list.append(mask_tensor.squeeze().cpu().numpy())

        # Denormalization for visualization
        # Assuming 'data' is a tensor of shape [batch_size, 6, channels, height, width]
        for images in stacked_images:
            for img in images:
                test_imgs.append(denormalization(img.cpu().numpy()))
    ...
    if is_vis:
        plot_sample_cv2(test_imgs, {'sum':scores}, gt_mask_list, save_folder=img_dir)
...
def main(args):
    ...
    dataset = '/autodl-fs/data/multi_lighting/all_lighting'
    # dataset = 'E:/DATASET/multi_lighting/all_lighting'
...
def get_args():
    ...
    parser.add_argument('--class_name', type=str, default='PeppermintCandy')
